...
def boar_brawl(player_score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    opponent_term = (opponent_score % 10 ** 2) // 10
    player_term = player_score % 10
    if opponent_score < 10:
        opponent_term  = 0
    return max(3 * abs(opponent_term - player_term), 1)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score0 in range(0, goal):
        for score1 in range (0, goal):
            if strategy(score0, score1) == ref:
                count += 1 
    return count == possiblities


def make_averaged(func_to_average, iterations=1000):
    """Return a function that returns the average value of func_to_average
    called iterations times.
    ...
    def calc_avg(*args):
        ...
        total = 0
        while i < iterations:
            total = total + func_to_average(*args)
            i += 1
...
def max_scoring_num_rolls(dice=six_sided, iterations=1000):
    ...
    current = 0
    maximum = 1
    averaged = make_averaged(roll_dice, iterations)
    for i in range (1, 11):
        current = averaged(i, dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Sus Fuss rules."""
    "*** YOUR CODE HERE ***"
    score_with_fuss = sus_update(0, score, opponent_score)
    if (score_with_fuss - score) >= threshold:
        return 0
